...
export const parsePage = (page: string): Game[] => {
    ...
    const ranks = (select(GAME_RANKS_X_PATH, dom) as any[]).map(
        (selectedValue: any) => getString(selectedValue)
    );
    const namesYears = (select(GAME_NAMES_YEARS_X_PATH, dom) as any[]).map(
        (selectedValue: any) => getString(selectedValue)
    );
    const ids = (select(GAME_LINK_X_PATH, dom) as any[]).map(
        (selectedValue: any) => getIdFromLink(getString(selectedValue))
    );

    const names: string[] = [];
    const years: string[] = [];

    namesYears.forEach((nameYear: any) => {
